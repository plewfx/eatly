...
const Restaurant = ({ id, name, highlight, img, time, rating, marked }) => {
    ...
    return (
        <div data-aos='fade-right' data-aos-delay={`${id * 100}`} className="restaurant">
            <Link to={`/menu/${name.toLowerCase().replace(/\s/g, '-')}`} className="restaurant__image">
                <img src={`/src/assets/restaurants/${img}.png`} alt={name} />
            </Link>
            <Link to={`/menu/${name.toLowerCase().replace(/\s/g, '-')}`} className="restaurant__info">
            ...
            <button onClick={toggleMarked}>
                <img src={'/src/assets/icons/bookmark' + (marked ? '-active.svg' : '.svg')} alt="Save" />
            </button>
        </div>
    )
}
